...
Suppose we want to compare outcomes with 3 treatments, and our data look like this
...
Data in wide format aren't convenient for running our analyses: if we want to run an ANOVA or Tukey, we need to have all the outcomes in one column, and the categories (treatments) in another column. This alternative formatting is called "long format". We can go from wide to long format using `gather` in `library(tidyr)`. 

```{r}
data %>% gather(key=treatment, value=outcome, Treat1, Treat2, Treat3)
```

The first argument in `gather` is for naming the new column that contains the categories (the `key`), the second one is for naming the column where the new outcomes will be stored (the `value`), and then we write the names of the columns that contain the outcomes. An equivalent way of writing the same thing is:
...
Note that `data2` isn't just our dataset that came out of `gather`ing. In fact, if we start with

```{r}
gath = data %>% gather(key=treatment, value=outcome, Treat1:Treat3)
```

and we try to `spread`, we'll get an error

```{r, eval=FALSE}
gath %>% spread(treatment, outcome) 
```

`tidyr` complains because the rows of `gath` aren't uniquely identifiable. A way to get around that is creating index variables within the treatments
...
You would like to create a new column that merged the information in `spirit` and `mixer`. You can use the command `unite` to do that.
...
The function `trimws` got rid of some empty spaces at the beginning and ends of strings. 
...
You can split up variables using `separate`. For example, if we start with
...
If we used the dataset `final` instead, we would have to do more work to `separate` correctly.


## Summarizing what we know with `gapminder`
...
And we can plot the trends as follows:
...
Let's do something a little more difficult. Let's try to find the population differential between 2007 and 1957 (range of the data) and sort the data in ascending order by the population differential. A way to do this is
...
Notice the backticks in the variables when we define `diffPop`. 
