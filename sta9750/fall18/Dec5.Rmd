...
```{r}
left_join(nonalcoholic, alcoholic, by='name') %>% select(name, spirit, mixer)
```

```{r}
right_join(nonalcoholic, alcoholic, by='name') %>% select(name, spirit, mixer)
```

```{r}
inner_join(nonalcoholic, alcoholic, by='name') %>% select(name, spirit, mixer)
```

```{r}
full_join(nonalcoholic, alcoholic, by='name') %>% select(name, spirit, mixer)
```
...
We can solve this problem in different ways. One way is using `coalesce` to replace the `NA`s by 0s in `price.x` and `price.y`, and then adding:
