...
We will use the `diamonds` dataset in `library(ggplot2)`, so we load the library and load the dataset.
...
Sometimes I feel like the default fontsize of the plots is too small. You can update the plot as follows
...
Some other things that are easy to change are the color of the bars
...
And you can flip the coordinates as follows
...
As you saw, since we were saving the changes as we produced them, all the previous changes to the plot were saved. As you might imagine, an equivalent code to produce the plot is
...
The $p$-value is clearly significant, although the boxplots didn't look all that different. The reason is that the sample size is big (`r nrow(diamonds)` observations), so small differences are flagged as significant.
...
We can fit some linear trend

```{r}
qplot(x=carat, y=price, data=diamonds)+geom_smooth(method='lm')
```

**Exercise** Can we try to use transformations to make the linear fit better?

**Solution** Yes, if we take logs of both variables, it'll look better. There are still some patches which are probably due to the existence of some *hidden* variable(s). The variance doesn't seem to be constant.
